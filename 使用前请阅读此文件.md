...
- data  存放测试数据路径 2-c
- logs  存放测试日志目录 2-d
- report    测试报告生成目录，目前支持生成两种形式的报告 5
- testcase  存放测试用例文件目录 2
- venv  本框架使用的虚拟环境 1
- conftest.py 全局操作，名称是固定写法不可更改 4-a 
- environment.xml allure测试报告总览-环境显示内容 5-a
- extract.yaml 接口依赖参数存放文件 2-b
- pytest.ini pytest框架规范约束，名称是固定写法不可更改 2-a
- requirements.txt 本框架所使用的到的第三方库 1-a 
- run.py 主程序入口 6
